...
def is_whisper_available() -> bool:
    ...
    try:
        result = subprocess.run(
            ["whisper", "--help"], capture_output=True, text=True, timeout=10
        )
...
def detect_language(video_path: Path, model: str = "base") -> Optional[str]:
    ...
    if not is_whisper_available():
        raise SubtitleError(
            "Whisper n'est pas installé. Installez avec: pip install openai-whisper"
        )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if result.returncode != 0:
                log.warning(
                    "Échec extraction audio pour détection langue: %s", result.stderr
                )
                return None
...
def generate_subtitles(
    ...
    try:
        ...
        if expected_srt.exists():
            ...
            raise SubtitleError(
                f"Fichier SRT non trouvé après génération: {expected_srt}"
            )
    ...
    for lang in languages:
        try:
            ...
            if lang == source_language:
                ...
                log.warning(
                    "Traduction vers %s non supportée directement. Utilisez un service de traduction externe.",
                    lang,
                )
                continue
