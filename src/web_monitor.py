...
class TaskMonitor:
    ...
    def get_task_stats(self) -> Dict[str, Any]:
        ...
        archived_tasks = self.get_archived_tasks(100)  # Plus large échantillon pour stats
        ...
        for task in archived_tasks:
            try:
                received_at = datetime.fromisoformat(task.get('received_at', '').replace('Z', '+00:00'))
...
def run_server(queue_dir: str, archive_dir: str, host: str = "127.0.0.1", port: int = 8000):
    """Lancer le serveur web"""
    app = create_app(queue_dir, archive_dir)
    
    print(f"🚀 Démarrage du serveur de monitoring...")
