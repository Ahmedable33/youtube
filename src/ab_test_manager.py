...
class ABTestManager:
    ...
    def _generate_variants(
        self, base_metadata: Dict, trending_keywords: List[str]
    ) -> Dict[str, ABVariant]:
        ...
        if trending_keywords:
            trending_title = self._add_trending_keyword(
                base_title, trending_keywords[0]
            )
    ...
    def _complete_test(self, test: ABTest):
        ...
        log.info(
            f"Test A/B terminé: {test.test_id}, gagnant: {test.winner} (confiance: {test.confidence:.2%})"
        )
        ...
        for test_file in self.storage_path.glob("ab_*.json"):
            try:
                ...
                if (
                    ...
                    and test.created_at < cutoff_date
                ):
                    test_file.unlink()
                    cleaned += 1
            except Exception as e:
