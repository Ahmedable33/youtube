...
async def cmd_account_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not Path(credentials_path).exists():
            await msg.reply_text(f"❌ Fichier credentials introuvable: {credentials_path}")
            return
        ...
        account = YouTubeAccount(
            ...
            daily_quota_limit=config.get("multi_accounts", {}).get("default_limits", {}).get("daily_quota_limit", 10000),
            daily_upload_limit=config.get("multi_accounts", {}).get("default_limits", {}).get("daily_upload_limit", 6),
            enabled=True
        )
...
async def cmd_account_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if manager.set_chat_account(str(msg.chat_id), account_id):
            ...
            await msg.reply_text(f"❌ Impossible de sélectionner le compte: {account_id}")
...
async def cmd_account_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if status['last_upload']:
            ...
            lines.append(f"🕒 Dernier upload: {last_upload.strftime('%d/%m/%Y à %H:%M')}")
