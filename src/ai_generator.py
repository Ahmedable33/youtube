...
def generate_metadata(req: MetaRequest, config_path: str = "config/video.yaml", video_path: Optional[str] = None) -> dict:
    ...
    if not seo_cfg:
        try:
            ...
            seo_cfg = {}
    default_provider = (seo_cfg.get("provider") or os.environ.get("SEO_PROVIDER") or "openai")
    ...
    if provider == "ollama":
        ...
        seo_timeout = None
        try:
            seo_num_predict = int(os.environ.get("OLLAMA_NUM_PREDICT")) if os.environ.get("OLLAMA_NUM_PREDICT") else None
        except Exception:
            seo_num_predict = None
        try:
            seo_timeout = int(os.environ.get("OLLAMA_TIMEOUT")) if os.environ.get("OLLAMA_TIMEOUT") else None
        except Exception:
            seo_timeout = None
        # Lire depuis YAML si présent
        if isinstance(seo_cfg, dict):
            if seo_cfg.get("num_predict") is not None and isinstance(seo_cfg.get("num_predict"), int):
                seo_num_predict = seo_cfg.get("num_predict")
            if seo_cfg.get("timeout_seconds") is not None and isinstance(seo_cfg.get("timeout_seconds"), (int, float)):
    ...
    if provider == "none":
        ...
        try:
            metadata = _ollama_generate_fast(req, vision_analysis) if fast_mode else _ollama_generate(req, vision_analysis)
        except Exception:
...
def _ollama_generate_fast(req: MetaRequest, vision_analysis: Optional[Dict] = None) -> dict:
    ...
    title = (title_out.splitlines()[0] if title_out else (req.topic or "Nouvelle vidéo"))[: req.max_title_chars]
    ...
    if isinstance(tags_data, list):
        ...
        tags = [str(t).strip().lstrip('#').lower() for t in tags_data.get("tags", []) if str(t).strip()]
    tags = tags[: req.max_tags]

    # Catégorie heuristique (rapide) si demandé
    result = {
        "title": title or (req.topic[: req.max_title_chars] if req.topic else "Nouvelle vidéo"),
