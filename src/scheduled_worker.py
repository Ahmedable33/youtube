...
class ScheduledWorker:
    ...
    def __init__(
        ...
        self.scheduler = UploadScheduler(
            config_path=Path("config/video.yaml"), schedule_dir=schedule_dir
        )
...
def run_scheduled_worker(
    schedule_dir: str, queue_dir: str, archive_dir: str, check_interval: int = 60
):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: python scheduled_worker.py <schedule_dir> <queue_dir> <archive_dir> [check_interval]"
        )
        sys.exit(1)
