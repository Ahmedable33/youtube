...
def upload_caption(
    ...
    try:
        ...
        media = MediaFileUpload(
            str(srt_path), mimetype="application/octet-stream", resumable=True
        )

        log.info("Upload sous-titres %s pour vidéo %s", language, video_id)

        request = youtube.captions().insert(
            part="snippet", body=caption_metadata, media_body=media
        )
...
def update_caption(
    ...
    try:
        ...
        current = youtube.captions().list(part="snippet", id=caption_id).execute()
        ...
        request = youtube.captions().update(
            part="snippet", body=updated_metadata, media_body=media
        )
...
def upload_multiple_captions(
    credentials, video_id: str, subtitle_files: Dict[str, Path], is_draft: bool = False
) -> Dict[str, Any]:
    """
