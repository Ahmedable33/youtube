...
class UploadScheduler:
    ...
    def __init__(self, config_path: Path, schedule_dir: Path, timezone: str = "Europe/Paris"):
            ...
            try:
                with open(self.slots_file, 'r', encoding='utf-8') as f:
                    ...
                    for day, slots in data.items():
                        self.time_slots[day] = [TimeSlot.from_dict(slot) for slot in slots]
    ...
    def find_next_optimal_slot(self, 
                              from_time: Optional[datetime] = None,
                              preferred_days: Optional[List[str]] = None,
                              min_delay_hours: int = 1) -> datetime:
        """
    ...
    def _is_slot_available(self, slot_time: datetime, slot: TimeSlot, max_tasks_per_slot: int = 3) -> bool:
        ...
        for task in self.scheduled_tasks:
            if (task.status in [ScheduleStatus.SCHEDULED, ScheduleStatus.READY] and
                slot_time <= task.scheduled_time <= slot_end):
                tasks_in_slot += 1
    ...
    def schedule_task(self, 
                     ...
                     preferred_days: Optional[List[str]] = None) -> ScheduledTask:
        """
        Planifier une tâche
    ...
    def get_ready_tasks(self, current_time: Optional[datetime] = None) -> List[ScheduledTask]:
        ...
        ready_tasks = []
        for task in self.scheduled_tasks:
            if (task.status == ScheduleStatus.SCHEDULED and 
        ...
        for task in self.scheduled_tasks:
            if task.task_id == task_id:
                ...
                if retry and task.attempts < task.max_attempts:
                    ...
                    task.scheduled_time = datetime.now(self.timezone) + timedelta(hours=1)
                    task.status = ScheduleStatus.SCHEDULED
                    log.info(f"Tâche reprogrammée: {task_id} (tentative {task.attempts})")
                else:
    ...
    def cleanup_old_tasks(self, days_old: int = 30):
        ...
        self.scheduled_tasks = [
            ...
            if not (task.status in [ScheduleStatus.COMPLETED, ScheduleStatus.FAILED] and
                   task.created_at and task.created_at < cutoff_date)
        ]
