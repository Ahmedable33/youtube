...
def test_subtitles_e2e(monkeypatch, tmp_path: Path):
    ...
    uploaded_ref = {}
    def _fake_upload_captions(credentials, video_id, subtitle_files, replace_existing, is_draft):
        # Capture what was uploaded
        uploaded_ref["files"] = dict(subtitle_files)
        # Return success for all langs provided
        return {lang: {"success": True, "action": "insert", "caption_id": f"cap_{lang}"} for lang in subtitle_files}
